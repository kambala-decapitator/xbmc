...
bool CMainKeyboard::ShowAndGetInput(char_callback_t pCallback,
                                    ...
                                    bool bHiddenInput)
{
  ...
  @synchronized([KeyboardView class])
  {
    ...
    CGRect keyboardFrame =
        CGRectMake(0, 0, pCurrentScreen.bounds.size.height, pCurrentScreen.bounds.size.width);
...
bool CMainKeyboard::SetTextToKeyboard(const std::string& text, bool closeKeyboard /* = false */)
{
  if (!g_pTvosKeyboard)
    return false;
  [g_pTvosKeyboard setKeyboardText:[NSString stringWithUTF8String:text.c_str()]
