...
if [ "${PLATFORM_NAME}" == "iphoneos" ] || [ "${PLATFORM_NAME}" == "appletvos" ]; then
  ...
  if [ "${PLATFORM_NAME}" == "iphoneos" ]; then
    ...
    if [ -f ${LDID} ]; then
      find ${BUILT_PRODUCTS_DIR}/${FULL_PRODUCT_NAME}/ -name "*.dylib" | xargs ${LDID} -S${IOS11_ENTITLEMENTS}
      find ${BUILT_PRODUCTS_DIR}/${FULL_PRODUCT_NAME}/ -name "*.so" | xargs ${LDID} -S${IOS11_ENTITLEMENTS}
