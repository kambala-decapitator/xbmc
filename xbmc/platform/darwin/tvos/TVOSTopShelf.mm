...
void CTVOSTopShelf::SetTopShelfItems(CFileItemList& movies, CFileItemList& tv)
{
  ...
  auto fillSharedDicts = [&](CFileItemList& videoItems, NSString* videosKey,
                             NSString* videosTitleKey, uint32_t titleStringCode,
  ...
  fillSharedDicts(
      movies, @"movies", @"moviesTitle", 20386,
  ...
  fillSharedDicts(
      ...
      [](CFileItemPtr videoItem) {
        return StringUtils::Format(
            ...
            videoItem->GetVideoInfoTag()->m_iSeason, videoItem->GetVideoInfoTag()->m_iEpisode);
      });
  videoDb.Close();
